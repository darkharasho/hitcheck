...
- Produces:
  - `startCapture(sourceId: string): Promise<MediaStream>`
...
/**
 * Acquire a capture stream. On Wayland the compositor's portal picker
 * owns source selection, so `sourceId` is a hint that the main-process
 * display-media handler may or may not be able to honour.
 */
export async function startCapture(sourceId: string): Promise<MediaStream> {
  ;(window as unknown as { __hitcheckPreferredSource?: string }).__hitcheckPreferredSource = sourceId
...
document.getElementById('start')!.addEventListener('click', async () => {
  ...
  const stream = await startCapture(sources[0]?.id ?? '')
