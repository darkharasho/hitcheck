...
- **Card image variant is `images.small`** (~245×342), not `images.large`. Small averages ~160KB/card, **~3.1GB total** (measured 2026-08-10; an earlier ~800MB estimate was wrong). Large would be several times that for no benefit, since DINOv2 ingests 224×224.
...
def download_images(pairs, root, fetch, sleep=time.sleep, max_attempts=4, on_progress=None):
    ...
    for i, (card_id, url) in enumerate(pairs, start=1):
        ...
        # Require non-zero size, not just existence: a zero-byte file would
        # otherwise be treated as permanently downloaded and never retried.
...
Expected: 20,479 cards; image directory roughly 3.1GB (measured average 160KB/card).
...
def motion_blur(image: Image.Image, seed: int, strength: float) -> Image.Image:
    ...
    MUST be DIRECTIONAL, not a gaussian. A gaussian is angle-invariant, so it
    cannot produce the anisotropic detail loss real hand-shake causes — fine
    print smears along one axis while perpendicular structure stays crisp.
    DINOv2 is texture-sensitive, so a symmetric blur removes proportionally
    less discriminative signal and makes M2's accuracy look better than it is.

    Implementation: seed an angle from _rng(seed + 2), build an odd-length
    normalized line kernel sized from `strength`, convolve with edge padding.
    Note PIL's ImageFilter.Kernel only supports 3x3/5x5 — larger kernels need
    a numpy convolution. Test by measuring gradient magnitude across
    perpendicular edges: they must differ substantially (isotropic gives a
    ratio of exactly 1.0, which is how the original bug was caught).
    """
